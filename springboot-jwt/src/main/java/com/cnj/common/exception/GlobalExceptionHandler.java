...
public class GlobalExceptionHandler {
    ...
    @ResponseBody
    public ResponseData handleException(HttpServletRequest request, HttpServletResponse response, Throwable ex) {
        log.error("handleException :{}",ex);
        if (HttpRequestUtil.isJsonRequest(request)){
            return ResultUtils.error(ex.getMessage());
        }
        if ("org.springframework.security.access.AccessDeniedException".equals(ex.getClass())){
            try {
